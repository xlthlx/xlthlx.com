...
/**
 * Sorts by term taxonomy.
 *
 * @param string $orderby The ORDER BY clause of the query.
 * @param object $query The WP_Query instance (passed by reference).
 *
 * @return string The modified ORDER BY clause of the query.
 */
function xlt_sort_film_tv_in_years( $orderby, $query ) {
	if ( ! xlt_is_year_tax_query( $query ) ) {
		return $orderby;
	}

	global $wpdb;
	return "$wpdb->term_relationships.term_order DESC";
}

/**
 * Set the sort taxonomy.
 *
 * @param object $query The WP_Query instance (passed by reference).
 *
 * @return bool|void
 */
function xlt_is_year_tax_query( $query ) {
	if ( is_admin() ) {
		return;
	}
	...
	return 'year' === $query->tax_query->queries[0]['taxonomy'];
}

add_filter( 'posts_orderby', 'xlt_sort_film_tv_in_years', 99, 2 );
