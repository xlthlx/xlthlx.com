...
function xlt_film_init() {
	...
	$film = 'data:image/svg+xml;base64,' . base64_encode(
...
function xlt_film_updated_messages( $messages ) {
	...
	$messages['film'] = array(
		...
		9  => sprintf(
			'Film scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview Film</a>',
...
function xlt_tv_series_init() {
	...
	$tv = 'data:image/svg+xml;base64,' . base64_encode(
		'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
	<path fill="#FFFFFF"
...
function xlt_tv_series_updated_messages( $messages ) {
	...
	$messages['tv-series'] = array(
		...
		9  => sprintf(
			'TV Series scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview TV Series</a>',
			date_i18n( 'M j, Y @ G:i', strtotime( $post->post_date ) ),
			esc_url( $permalink )
		),
